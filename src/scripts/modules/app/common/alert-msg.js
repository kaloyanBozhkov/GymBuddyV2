...
export default (fileToAppend, smoothSwitch = true, replaceWhat = [], replaceWith = [], attributes = [], noDisplayMessageBackgroundColor=false) => { //attributes is array of objects with attrName = value
    ...
    let content = `<div id='displayMessage'${noDisplayMessageBackgroundColor ? 'class="displayMessage--noBackgroundColor"' : ''}>${global.msgBox[fileToAppend]}</div>`;
    ...
    if (!smoothSwitch) {
        $("body").prepend(`<div id='alertBg' class='noSelect'>${content}</div>`);
        $("#alertBg").animate({
            "opacity": "1"
        }, 100);
    } else {
